...
parser.add_argument("--seen_file", type=str, default="../vectors/bow2_distrib.vectors", help="Seen vectors file")
parser.add_argument("--adjusted_file", type=str, default="../vectors/bow2_ar.vectors", help="Adjusted vectors file")
parser.add_argument("--unseen_file", type=str, default="../vectors/bow2_prefix.vectors", help="Unseen vectors file")
parser.add_argument("--out_dir", type=str, default="../save/", help="Where to store experiment logs and models")
...
# mapping
parser.add_argument("--map_type", type="str", default="linear", help="Mapping types available: linear")
parser.add_argument("--map_id_init", type=bool, default=True, help="Initialize the mapping as an identity matrix")
parser.add_argument("--orthogonalize", type=bool, default=True, help="Orthogonalize the weight")
# discriminator
...
def initialize_exp(params):
    ...
    # create logger
    logging.basicConfig(os.path.join(params.exp_path, 'train.log'), level=getattr(logging, params.verbose.upper()))
    logging.info('============ Initialized logger ============')
    logging.info('\n'.join('%s: %s' % (k, str(v)) for k, v in sorted(dict(vars(params)).items())))
    logging.info('The experiment will be stored in %s' % params.exp_path)
